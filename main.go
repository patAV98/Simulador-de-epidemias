...
func main() {
	...
	medIntroduced = flag.Int("m", 50, "day when medicine is introduced")
	medEffectiveness = flag.Float64("e", 0.50, "effectiveness of medicine")
	qIntroduced = flag.Int("q",50, "day when quarantine is introduced")
	qEffectiveness = flag.Float64("g", 0.2, "effectiveness of quarantine")
	...
	for t = 0; !endSim && (t < *numDays); t++ {
		...
		f, err := os.Create(fmt.Sprintf("./images/%d.png", cuenta))
		if err != nil {
    		panic(err)
		}
		defer f.Close()
...
func saveData(name string) {
	...
	cellsfile, err := os.Create(fmt.Sprintf("./epidemic-%s.csv", name))
	if err != nil {
